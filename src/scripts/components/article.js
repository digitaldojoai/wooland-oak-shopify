...
class PassportStory extends Component {
  constructor (props) {
    super(props)
    this.state = {
  ...
  componentDidMount () {
    ...
    // FIXME: Figure out the prev/next story
    // const prevStory = stories.prev(this.props.slug)
  ...
  renderModules (modules) {
    return modules.map((module) => {
      ...
      if (type === 'moduleYouMayLike') return
    ...
    if (story) {
      ...
      return (
        <div className='single'>
          <Helmet
            title={`${passportTitle} - Wool & Oak`}
            meta={[
              {property: 'og:url', content: `https://www.woolandoak.com/blogs/passport/${slug}`},
              {property: 'og:image', content: `${featuredImage.fields.file.url}?fm=jpg&fl=progressive`},
              {property: 'twitter:image', content: `${featuredImage.fields.file.url}?fm=jpg&fl=progressive`}
            ]}
            link={[
              {rel: 'canonical', href: `https://www.woolandoak.com/blogs/passport/${slug}`}
            ]}
          />
          <div className='article__hero rel'>
            <ImageFill image={featuredImage} />
          </div>
          <section className='bgw z0 rel'>
            <div className='rel pt2'>
              <div className='outer container--c container--a'>
                <h4 className='mb0 mv05 book'>{passportTitle}</h4>
                <div>
                  <p className='small mx0 sans caps bold px0'>{tags}</p>
                  {author && (<span className='small sans bold'>{author} - </span>)}
                  <span className='small sans cgd bold'>{moment(publishedAt).format('MM.DD.YYYY')}</span>
                </div>
              </div>
            </div>
          </section>
          <div className='z0 bgw pv2'>
            <div className='article__nav_block z2'>
              <div className='outer container--c container--a'>
                ...
                <a href='' className='co social mr1' data-social='twitter' data-url={`https://www.woolandoak.com/blogs/passport/${slug}`}><Twitter /></a>
                <a href='' className='co social mr1' data-social='facebook' data-url={`https://www.woolandoak.com/blogs/passport/${slug}`}><Facebook /></a>
                <a className='co social' href={`mailto:name@place.com?subject=${passportTitle}&body=https://www.woolandoak.com/blogs/passport/${slug}`}><Email /></a>
              </div>
            </div>
            { this.renderModules(story.contentModules) }
            <div className='outer pv2 container--c container--a'>
              <div className='article__social_footer'>
                <a href='' className='co social mr2' data-social='twitter'><Twitter /></a>
                <a href='' className='co social mr2' data-social='facebook'><Facebook /></a>
                <a className='co social' href={`mailto:name@place.com?subject=${passportTitle}&body=https://www.woolandoak.com/blogs/passport/${slug}`}><Email /></a>
              </div>
            </div>
          </div>
...
export default root => {
  ...
  stories.on('updated', () => render(<PassportStory slug={slug} />, root))
  render(<PassportStory slug={slug} />, root)
}
